...
class ProfessorController {
  async index(request, response) {
    const professors = await User.query()
      .innerJoin("professors", "professors.userEmail", "users.email")
  ...
  async getProposalsToReview(request, response) {
    ...
    const proposalsData = await Proposal.query()
      .withGraphJoined(
        "[class, student.user(filterUser), reviews.reviewer.user(filterUser)]"
      )
      .modifiers({
        filterUser: (builder) => {
    ...
    proposalsData.forEach((proposal) => {
      const status =
        proposal.reviews.length <= 0
          ? "Pendente"
          : ProposalService.proposalStatus(proposal);
  ...
  async getProposalsToCoordinate(request, response) {
    ...
    const proposals = await Proposal.query()
      .withGraphJoined("[class(filterClass), reviews.reviewer.user]")
      .modifiers({
        filterClass: (builder) => {
  ...
  async getAllClasses(request, response) {
    ...
    const coordinator = await Class.query()
      .innerJoin(
        "professors",
        "professors.userEmail",
        "classes.coordinatorEmail"
      )
      .select(
        "classes.id",
        "classes.name",
    ...
    const advisor = await Class.query()
    ...
    const reviewer = await Class.query()
      .innerJoin("proposals", "proposals.classId", "classes.id")
      .select(
        "classes.id",
        "classes.name",
  ...
  async create(request, response) {
    ...
    const verifyProfessor = await User.query().findById(professor.email);

    if (verifyProfessor) {
      return response
        .status(400)
        .json({ status: "Usuário com este email já cadastrado." });
    }
    ...
    const insertProfessor = await Professor.query(trx).insert({
    ...
    if (!insertProfessor) {
      return response
        .status(500)
        .json({ status: "Erro ao adicionar professor." });
    }

    return response
      .status(200)
      .json({ status: "Professor adicionado com sucesso." });
  }
  ...
  async update(request, response) {
    ...
    return response
      .status(200)
      .json({ status: "Professor modificado com sucesso." });
  }

  async delete(request, response) {
    const professor = request.body;

    const professorExists = await User.query().where("email", professor.email);

    if (professorExists.length === 0) {
      return response.status(400).json({ status: "Professor não encontrado." });
    }
    const professorProposals = await Proposal.query().where(
      "advisorEmail",
      professor.email
    );
    ...
    if (professorProposals.length > 0) {
      return response
        ...
        .json({
          status:
            "Professor já possui vínculo uma ou mais propostas no sistema, não é possível deletar.",
        });
    }

    const professorReviews = await Review.query().where(
      "reviewerEmail",
      professor.email
    );
    ...
    if (professorReviews.length > 0) {
      return response
        ...
        .json({
          status:
            "Professor já possui vínculo uma ou mais revisões no sistema, não é possível deletar.",
        });
    }

    const professorClasses = await Class.query().where(
      "coordinatorEmail",
      professor.email
    );
    ...
    if (professorClasses.length > 0) {
      return response
        ...
        .json({
          status:
            "Professor já possui vínculo uma ou mais turmas no sistema, não é possível deletar.",
        });
    }

    await User.query().deleteById(professor.email);

    return response
      .status(200)
      .json({ status: "Professor deletado com sucesso." });
  }
}
